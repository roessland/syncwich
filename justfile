...
# Show current test coverage for each file in rd package
show-coverage:
    @echo "📊 Test coverage by file in rd/ package:"
    go test -coverprofile=coverage.out ./rd >/dev/null 2>&1
    go tool cover -func=coverage.out | grep "rd/.*\.go:"
    @rm -f coverage.out
...
clean:
    ...
    rm -rf rd/testdata/fixtures/*
    rm -rf rd/testdata/golden/* 